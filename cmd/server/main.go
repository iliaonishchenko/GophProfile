...
func run() error {
	...
	avatarService := service.NewAvatarService(repo, objectStorage, rabbit, cfg.PublicBaseURL)
	healthService := service.NewHealthService(db, objectStorage, rabbit)
	handler := httpserver.NewHandler(avatarService, healthService)
	...
	server := &http.Server{Addr: cfg.HTTPAddress, Handler: router, ReadHeaderTimeout: cfg.ShutdownPeriod}
